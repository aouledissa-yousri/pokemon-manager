...
import { useAddTrainerDialogStore } from "./add-trainer.dialog.component.store"


export function AddTrainerDialogComponent() {

    const toast = useToast()

    const open = useAddTrainerDialogStore(state => state.open)
    const closeDialog = useAddTrainerDialogStore(state => state.closeDialog)
    ...
    const { control, formState, handleSubmit, reset } = useForm<TrainerFormValidationSchemaInput, unknown, TrainerFormValidationSchemaElements>({
    ...
    const handleClose = () => {
        reset(TrainerFormValidationSchema.parse({}))
        setImageFile(null)
        closeDialog()
    }
    ...
    const onSubmit = async (values: TrainerFormValidationSchemaElements) => {
        ...
        try {
            const response = await trainerProxy.addTrainer({ name: values.name, imageFile })

            if (response.success) {
                toast.show("Trainer added", "success")
            ...
            toast.show("Failed to add trainer", "error")
        }
